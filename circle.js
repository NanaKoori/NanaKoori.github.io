...
function preload() {
    ...
    table_qx=loadTable("./asset/qx_sum.csv",'csv','header');
    table_dg=loadTable("./asset/dg_sum.csv",'csv','header');
    table_mt=loadTable("./asset/mt_sum.csv",'csv','header');
    table_sy=loadTable("./asset/sy_sum.csv",'csv','header');
    table_zy=loadTable("./asset/zy_sum.csv",'csv','header');
...
function draw() {
    background(255);
    chartInfo();
    diagramX=(width/4)*3-60;
    diagramY=height/2+15;
    let radius=width/5-60;
    ...
    function chartInfo(){
        ...
        fill('black');
        textFont('Georgia');
        text("To provide a more intuitive and interactive display, this section utilizes p5.js to visualize the daily workload. The data is presented in a circular-like format, with each member of the group represented by an arc. \nEach individual will be assigned a unique color and his/her workload at a specific time is represented by a straight line extending outward from the arc, with varying lengths of these lines roughly reflecting a visualization of the distribution of workload across the individuals represented in the display.\n By hovering over the dot at the edge of the line, the workload is displayed, and when an individual is on vacation (workload=0), the text vacation! is displayed.",width/5,height/4,width/4);
